def TryMove(nextGrid,shuriken,i,j):
    ...
    if nextGrid[i][j]+shuriken in ['rl','ab']: # Shuriken fly in opposite direction -> Crash
        #print(shuriken, 'Crashed Type-I: Shuriken fly into each other.')
        grid[i][j] = '-' # Destroy shuriken
        nextGrid[i][j] = '-'
        return nextGrid
    ...
    if nextGrid[i+vectorI][j+vectorJ] == '-': # No obstacle -> move
        ...
        if vectorI < 0 or vectorJ  < 0: # Don't crash with a shuriken that still doesn't move
            #print(shuriken, 'Crashed Type-II: Shuriken crash with old one.')
            grid[i+vectorI][j+vectorJ] = '-' # Destroy shuriken
            nextGrid[i+vectorI][j+vectorJ] = '-'
            return nextGrid
        elif nextGrid[i+vectorI][j+vectorJ]+shuriken in ['rl','ab']:
            #print(shuriken, 'Crashed Type-III: Shuriken crash with new one.')
            grid[i+vectorI][j+vectorJ] = '-' # Destroy shuriken
            nextGrid[i+vectorI][j+vectorJ] = '-'
            return nextGrid
        else:
